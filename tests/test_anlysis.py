...
def test_engagement_metrics():
    ...
    assert engagement_metrics.shape[0] == 3  # Three unique MSISDNs
    assert 'total_traffic' in engagement_metrics.columns  # Check total_traffic column
...
def test_cluster_assignment():
    ...
    assert len(clusters) == df.shape[0]  # Ensure all rows are assigned to a cluster
...
def test_top_users_per_app():
    ...
    assert top_users_per_app.shape[0] == 6  # 6 unique combinations of application and MSISDN
